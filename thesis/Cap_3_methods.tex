...
This chapter presents the ProSafe system architecture and the Adaptive Density-Aware Beaconing (ADAB) protocol, which form the case study for evaluating the simulator developed in this thesis. We begin by describing the overall system design and its operational requirements, then detail the communication model and beacon structure, and finally present both the static baseline protocol and the adaptive ADAB mechanism.

\section{System Overview}

ProSafe (Proximity-based Safety) is a minimalist, Wi-Fi-based safety network designed to enable vessels to detect people in the water through smart buoys that broadcast periodic, location-aware beacons. The system operates in a fully infrastructure-free manner within a local ad-hoc IEEE 802.11 domain, where all alerts are generated and handled locally without relying on central coordination or internet connectivity.

\subsection{Design Principles}

The ProSafe system is built around several key design principles that address the unique requirements of coastal safety applications:

\begin{enumerate}
    \item \textbf{Infrastructure-free operation:} No access points, routers, or internet connectivity required. The system must function in remote coastal areas where no infrastructure exists.
    
    \item \textbf{Pure broadcast communication:} All beaconing operates at the MAC layer using broadcast data frames, avoiding the complexity and overhead of association, routing, and IP-level protocols.
    
    \item \textbf{Passive vessel detection:} Vessels listen passively for buoy beacons without transmitting, maintaining simplicity and avoiding regulatory complications for receiving equipment.
    
    \item \textbf{Low-cost commodity hardware:} Leverage widely available IEEE 802.11 chipsets and devices (smartphones, single-board computers) to minimize deployment cost and maximize accessibility.
    
    \item \textbf{Energy efficiency:} Battery-powered buoys must operate for extended periods (hours) on limited energy, requiring careful management of transmission frequency and power consumption.
\end{enumerate}

\subsection{System Components}

The ProSafe architecture consists of two primary components, as illustrated in the paper:

\subsubsection{Smart User Buoys}

Smart User Buoys are compact floating devices carried or worn by swimmers, snorkelers, kayakers, or divers. Each buoy serves both as a visible marker and as an electronic beacon that advertises its presence and position through periodic Wi-Fi broadcasts while being towed during water activities.

A typical buoy integrates:
\begin{itemize}
    \item A single-board computer (e.g., Raspberry Pi) with Wi-Fi capability
...
The Vessel Safety Unit is installed on boats and operates exclusively in passive listening mode. When a buoy beacon is received with a signal strength exceeding a configurable RSSI (Received Signal Strength Indicator) threshold, the unit issues audible and visual alerts to the vessel operator.
...
\begin{itemize}
    ...
    \item \textbf{Timestamp:} Transmission time for latency calculation (4 bytes)
    ...
    \item \textbf{Neighbor list:} Recently heard neighbors with their timestamps and positions (28 bytes per neighbor)
\end{itemize}

The neighbor list enables indirect discovery: when buoy A receives a beacon from buoy B containing C in its neighbor list, A learns about C's existence and approximate position even if A has not directly received C's transmissions. This multi-hop awareness extends the effective detection range without requiring explicit forwarding protocols.

The total beacon size varies with the number of neighbors included but typically ranges from 100-500 bytes depending on local density.

\subsection{MAC Layer Operation}

Channel access follows the standard IEEE 802.11 Distributed Coordination Function (DCF) mechanism described in Chapter~\ref{chap:background}:

\begin{enumerate}
    \item \textbf{Carrier sense:} Before transmitting, a buoy senses the channel to determine if it is idle or busy
    \item \textbf{DIFS wait:} If the channel is idle, wait for a DCF Interframe Space (DIFS) period ($50~\mu$s)
    \item \textbf{Random backoff:} Select a random backoff counter from the contention window [0, CW-1] where CW = 16
    \item \textbf{Transmission:} When the backoff counter reaches zero and the channel remains idle, transmit the beacon frame
\end{enumerate}

Critically, \textbf{no RTS/CTS handshaking or acknowledgments} are used. This is appropriate for broadcast frames and keeps the protocol simple, but it also means that:
\begin{itemize}
    \item Senders have no feedback about delivery success
    \item Collisions are not detected by the sender
    \item The binary exponential backoff mechanism cannot adapt to collision events
    \item Hidden terminal problems can cause silent failures
\end{itemize}
...
To maximize over-water range and reliability, ProSafe targets widely available 2.4 GHz IEEE 802.11b/g/n chipsets and uses a robust legacy data rate:

\begin{itemize}
    \item \textbf{Frequency band:} 2.4 GHz (802.11b/g/n)
    \item \textbf{Data rate:} 1 Mbps using Direct Sequence Spread Spectrum (DSSS)
...
Buoys form a pure peer-to-peer ad-hoc domain (IBSS-style) with no access point, router, or DHCP server. The network is inherently dynamic:

\begin{itemize}
    \item Nodes enter and leave the communication range continuously due to water currents, waves, and mobility
    \item No persistent connections or associations are maintained
    \item Each node maintains a local neighbor table based on recently received beacons
    \item Neighbor entries expire if no beacon is heard within a timeout period (typically 3-5 times the beacon interval)
\end{itemize}

Beacons are primarily one-hop transmissions. However, to extend coverage, the system can optionally enable single-hop re-broadcasting: upon receiving a beacon, a node may retransmit it exactly once (TTL=1) to extend awareness to nodes that did not hear the original transmission. This provides limited multi-hop reach while bounding channel load.
...
SBP operation is straightforward:
...
\textbf{Advantages:}
\begin{itemize}
    \item Simple implementation with minimal computational overhead
    \item Predictable, deterministic behavior
    \item Discovery delay is bounded by $BI_{\text{static}}$
    \item Well-understood performance characteristics
\end{itemize}

\textbf{Limitations:}
\begin{itemize}
    \item Poor scalability: Channel load grows linearly with the number of nodes
    \item High collision rate in dense scenarios where many nodes transmit simultaneously
    \item No adaptation to changing network conditions
    \item Inefficient in sparse scenarios: Frequent transmissions waste energy when few neighbors are present
    \item Fixed interval cannot balance the competing goals of low latency and low congestion
\end{itemize}

As demonstrated in the results presented in the paper, SBP's Broadcast Packet Delivery Ratio (B-PDR) degrades significantly as node density increases, dropping from approximately 95\% at 20 nodes to around 70\% at 300 nodes. This degradation motivates the need for adaptive mechanisms.
...
The quadratic mapping $f_t^2$ is chosen deliberately to create nonlinear adaptation:
...
The random jitter $\epsilon_t$ is essential to prevent beacon synchronization. Without jitter, nodes experiencing similar density conditions would compute identical intervals and could synchronize their transmissions, leading to periodic collision bursts. The $\pm 5\%$ jitter introduces sufficient randomness to desynchronize nodes while maintaining the overall density-driven adaptation.
...
This clean separation means ADAB can be implemented on standard Wi-Fi hardware without requiring firmware modifications or low-level driver access.

\subsection{Comparison with Related Approaches}

Unlike complex adaptive beaconing schemes proposed for VANETs that may consider multiple factors (mobility, channel quality, application priority), ADAB intentionally remains simple:

\begin{itemize}
    \item Uses only one observable metric: neighbor count
    \item Requires no channel quality measurements or signal processing
    \item Operates without feedback or acknowledgments
    \item Involves minimal computation (suitable for resource-constrained devices)
    \item Functions in pure broadcast mode without coordination
\end{itemize}

This simplicity is a strength for the maritime safety use case, where robustness, predictability, and ease of implementation are paramount.
...
For ADAB, the hourly consumption depends on local density. In sparse scenarios ($N = 0$), ADAB consumes the same energy as SBP. In dense scenarios ($N \geq 15$), the interval increases to 5.0 s:
$$E_{\text{TX,h}}^{\text{ADAB}}(N \geq 15) = 3600 \times 0.2 \times 4.4 \text{ mJ} \approx 3.2 \text{ J/h}$$

This represents a \textbf{20× reduction in transmission energy} in high-density scenarios compared to fixed-interval beaconing, extending battery life significantly while maintaining or improving reliability.
...
\begin{itemize}
    ...
    \item Smart buoys periodically broadcast beacons containing position and neighbor information
    \item Vessel units passively listen and alert when buoys are detected within range
    \item Communication operates entirely at the MAC layer using broadcast frames without acknowledgments
    ...
    \item ADAB uses a quadratic mapping to aggressively throttle transmissions in dense scenarios
