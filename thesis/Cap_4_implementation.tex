...
The simulator is built around a discrete event simulation paradigm, where the system state evolves through a sequence of instantaneous events occurring at specific times.
...
\begin{enumerate}
    ...
    \item Determine reception probability based on distance, \textbf{?Here we could insert the probabilistic model that consider also the size of the packet?}:
    \begin{align*}
...
\begin{itemize}
    ...
    \item \textbf{Probabilistic channel:} Uses the calibrated distance based reception probabilities (\textbf{?Packet size?}). Models realistic over water propagation.
\end{itemize}
...
\subsection{Per Node Discovery Tracking (Multihop)}

The simulator tracks unique nodes discovered by each buoy for calculating the actual coverage of each protocol:
