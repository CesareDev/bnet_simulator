...
def plot_ramp_grouped_by_buoy_count(results_dir, plot_file):
    ...
    group_edges = [2, 6, 11, 16, 21, 26, 31, 36, 41, 46, 51]
    group_labels = [f"{group_edges[i]}-{group_edges[i+1]-1}" for i in range(len(group_edges)-1)]

    grouped_data = {mode: [] for mode, _ in modes}
    ...
    for i, (mode, color) in enumerate(modes):
        ax.bar(x + i * bar_width, grouped_data[mode], bar_width, label=mode.capitalize(), color=color)
